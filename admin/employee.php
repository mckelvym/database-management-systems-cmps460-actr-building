...
echo "<tr><td colspan=3>Employee Management</td></tr></thead>";	
echo "<tbody class=\"default\">";
echo "<tr><td><a href=\"employee_edit.php?id=0&mode=delall\">Delete All</a></td><td><a href=\"employee_edit.php?id=0&mode=add\">Add</a></tr>\n";
echo "</tbody></table>";
...
while($row = mysql_fetch_assoc($result)){	
	...
	$first = $row['firstname'];
	$pos = $row['position'];
	echo "<option value=\"employeeinfo.php?id=$id\">$first $last (".ucwords($pos).")</option>\n";
}
echo "</select></form>";
...
while($row = mysql_fetch_assoc($result)){	
	...
	$last = $row['lastname'];
	$first = $row['firstname'];
	echo "<option value=\"employeeinfo.php?id=$id\">$first $last</option>\n";	
}
echo "</select></form></td>";
mysql_free_result($result);

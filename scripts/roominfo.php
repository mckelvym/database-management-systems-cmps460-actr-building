...
Show information about a specific type of room.
-->
...
// if the room is an office we can show office hours for the occupant
...
// if the room is a lab we can show the pertinant information here
if($post_type == "lab"){
	...
	// type of the computers, the contact person, and hours of operation.
		...
		echo "<tr><td>Contact Person:</td><td><a href=\"employeeinfo.php?pageid=2&id=$id\">$first $last</a></td></tr>";
...
// if the room is a classroom we can show all classes that meet here.
	...
	if($yr == "" || $tm == ""){
		...
		echo "<table class=\"default\">";
		echo "<thead class=\"default\">";
		echo "<tr><td colspan=1 align=center>Term selection</td></tr></thead>";
		echo "<tbody class=\"default\">";
		echo "<tr>";
		...
		$query = "select year,term,level from class where room_num = '$post_info' order by year,term,level";
		$result = mysql_query((string)$query) or die('BAD QUERY '.mysql_error());
		while($row = mysql_fetch_assoc($result)){
			$year = $row['year'];
			$term = $row['term'];
			$level = $row['level'];
			echo "<option value=\"roominfo.php?pageid=3&room=$post_info&type=$post_type&yr=$year&tm=$term\">$term $year ($level)</option>\n";
		}
		echo "</select></form></td>";
		echo "</tr>";
		echo "</tbody></table>";
	...
	else{ // start year thing
	$query = "select year,term,course_num as CourseNum,course_id as CourseId,section,days,time,level,teacher_id from class where room_num = '$post_info' and year = '$yr' and term = '$tm' order by year,term,course_num,course_id,section,level";
	$result = mysql_query((string)$query) or die('BAD QUERY '.mysql_error());
	...
	while($i < mysql_num_rows($result)){
		...
		echo "<td>$time</td>";
		echo "<td>$level</td>";
		echo "<td><a href=\"employeeinfo.php?pageid=2&id=$id\">Instructor</a></td>";
		echo "</tr>";
		$i = $i + 1;
	}
	table_end();
...
// if the room is an office we can show office hours
...
// if the room is a seminar then we can show all scheduled seminars for this room
